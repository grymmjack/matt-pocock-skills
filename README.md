...
- **[env-browser](./skills/engineering/env-browser/SKILL.md)** — Explore a project's package environment the fuzzy way: browse installed packages, read their source, and jump to their registry/docs — for Python, Node, Rust, Go, and Ruby (`pybrowse`/`nodebrowse`/`rustbrowse`/`gobrowse`/`gembrowse`), siblings of `brewbrowse`/`aptbrowse`.
...
- **[learn-anything](./skills/productivity/learn-anything/SKILL.md)** — Drive a new topic through Rick's Learn Anything framework (the 5 Ws, resources, the continuum of learning, mastery) in a stateful, learner-driven per-topic workspace.
