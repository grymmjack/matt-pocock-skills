...
- **`/env-browser`** — explore a project's package environment interactively: it installs a family of fzf browsers (siblings of `brewbrowse`/`aptbrowse`) — **`pybrowse`** (Python), **`nodebrowse`** (npm), **`rustbrowse`** (Cargo), **`gobrowse`** (Go), **`gembrowse`** (Ruby gems) — each turning a dependency set into a navigable reading list where every package's purpose, source, and registry/docs sit one keystroke apart. Reach for it to understand what a codebase *depends on* rather than to change it; where `/research` reads the web, this reads the environment on your disk.
...
- **`/xmind-mindmap`** — turn a subject into a **real `.xmind` mind map** (single- or multi-sheet, hyperlinked, themed, with optional per-node screenshots/images, clickable links, and styling) built from a hierarchical topic tree, with optional SVG/PNG/PDF or interactive-HTML (markmap) renders. Model-invoked, so `/teach` and `/learn-anything` can pull it in to render a topic as a map into their workspace's `./reference/`. It **draws the map**; the learning skills run the learning.
