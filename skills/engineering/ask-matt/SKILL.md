...
- **`/xmind-mindmap`** — turn a subject into a **real `.xmind` mind map** (single- or multi-sheet, hyperlinked, themed) built from a hierarchical topic tree, with optional SVG/PNG/PDF renders. Model-invoked, so `/teach` and `/learn-anything` can pull it in to render a topic as a map into their workspace's `./reference/`. It **draws the map**; the learning skills run the learning.
