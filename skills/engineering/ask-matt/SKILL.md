...
- **`/python-env`** — explore a Python environment interactively: it installs **`pybrowse`**, an fzf browser (sibling of `brewbrowse`/`aptbrowse`) that turns `pip freeze` into a navigable reading list — every package with its purpose, source, and PyPI/docs one keystroke apart. Reach for it to understand what a codebase *depends on* rather than to change it; where `/research` reads the web, this reads the environment on your disk.
